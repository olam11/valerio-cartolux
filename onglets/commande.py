...
if style == "Atlas" or style == "Minimalist":
    ...
    format = st.radio(label="Format",options=["A5","A4","A3"])
...
with col1 :
    nb_elements_princ = st.number_input("Eléments principaux (Montagnes,ville,lac...)",min_value=1,max_value=10,step=1,help="Montagnes,ville,lac...")
    prix_total =prix_total+0.5*nb_elements_princ
with col2 :
    nb_elements_sec = st.number_input("Eléments secondaires (?)",min_value=1,max_value=10,step=1,help="?")
